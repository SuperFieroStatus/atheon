...
export function TaskModal(props: Props) {
  ...
  async function saveName() {
    if (name.trim() && name !== task.name) await props.onPatch(taskId, { name: name.trim() });
  }
  async function saveDesc() {
    if (desc !== task.description) await props.onPatch(taskId, { description: desc });
  }
  async function addSub() {
    if (!newSub.trim()) return;
    await props.onCreateSubtask(taskId, newSub.trim());
    setNewSub('');
  }
  ...
  async function createAndAddTag() {
    ...
    await props.onToggleTag(taskId, tag.id, true);
    setNewTag('');
  }
            ...
            <div className="hstack" style={{ marginBottom: 14, gap: 10 }}>
              <label className={'check-toggle' + (task.completed ? ' on' : '')}>
                <input
                  ...
                  disabled={!canEdit}
                  onChange={(e) => props.onPatch(taskId, { completed: e.target.checked } as any)}
                />
              ...
              <div className="tag-editor" ref={tagRef} style={{ position: 'relative' }}>
                {task.tags.map((t) => (
                  <span className="tag-chip" key={t.id} style={{ background: t.color }}>
                    {t.name}
                    {canEdit && <span className="x" onClick={() => props.onToggleTag(taskId, t.id, false)}>×</span>}
                  </span>
                ))}
                ...
                {tagMenu && (
                  <div className="tag-menu" style={{ top: 30, left: 0 }}>
                    {availableTags.map((t) => (
                      <button key={t.id} className="menu-item" onClick={() => { props.onToggleTag(taskId, t.id, true); }}>
              ...
              <div className="tag-editor" ref={assigneeRef} style={{ position: 'relative' }}>
                {assignees.map((m) => (
                  <span className="assignee-chip" key={m.id}>
                    ...
                    {canEdit && <span className="x" onClick={() => props.onRemoveAssignee(taskId, m.id)}>×</span>}
                  </span>
                ))}
                ...
                {assigneeMenu && (
                  <div className="tag-menu" style={{ top: 30, left: 0, maxHeight: 220, overflowY: 'auto' }}>
                    ...
                    {unassignedMembers.map((m) => (
                      <button key={m.id} className="menu-item" onClick={() => { props.onAddAssignee(taskId, m.id); setAssigneeMenu(false); }}>
                        <Avatar first={m.first_name} last={m.last_name} color={m.color} size="sm" />
            ...
            <div className="detail-grid" style={{ marginBottom: 14 }}>
              <div>
                ...
                <input type="date" className="meta-date" value={task.due_date || ''} disabled={!canEdit}
                  onChange={(e) => props.onPatch(taskId, { due_date: e.target.value || null } as any)} />
              </div>
              <div>
                <label className="field-label">Priority</label>
                <select className="meta-select" value={task.priority || ''} disabled={!canEdit}
                  onChange={(e) => props.onPatch(taskId, { priority: (e.target.value || null) as any })}>
              ...
              {isParent && (
                <div>
                  ...
                  <select className="meta-select" value={task.dependency_id || ''} disabled={!canEdit}
                    onChange={(e) => props.onPatch(taskId, { dependency_id: e.target.value || null } as any)}>
                  ...
                  {subtasks.map((s) => (
                    <div className={'st-item' + (s.completed ? ' done' : '')} key={s.id}>
                      <input type="checkbox" className="rcheck" checked={s.completed} disabled={!canEdit}
                        onChange={(e) => props.onPatch(s.id, { completed: e.target.checked } as any)} />
            ...
            <div className="comments">
              ...
              {canEdit && (
                <div className="comment-box">
                  ...
                  <button className="btn" onClick={postComment}>Send</button>
                </div>
              )}
            </div>
          </div>
